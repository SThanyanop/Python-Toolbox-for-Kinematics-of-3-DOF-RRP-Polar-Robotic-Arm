...
```
E_θ₃ = {(Pₘₐₚ(i,j,0), Pₘₐₚ(i,j,1)) : i ∈ {0, N₁-1} ∨ j ∈ {0, N₂-1}}
```

2. **Edges ตามแนว θ₂** (เชื่อมจุดที่ θ₂ ต่อเนื่องกัน):
```
E_θ₂ = {(Pₘₐₚ(i,j,k), Pₘₐₚ(i,j+1,k)) : i ∈ [0, N₁-1], j ∈ [0, N₂-2], k ∈ {0,1}}
```

3. **Edges ตามแนว θ₁** (เชื่อมจุดที่ θ₁ ต่อเนื่องกัน):
```
E_θ₁ = {(Pₘₐₚ(i,j,k), Pₘₐₚ(i+1,j,k)) : i ∈ [0, N₁-2], j ∈ [0, N₂-1], k ∈ {0,1}}
```
